...
function focusConcert(id, { flyTo = true, openPopup = true, scroll = true } = {}) {
  ...
  if (m && openPopup) {
    if (flyTo) setTimeout(() => m.openPopup(), 500);
    else m.openPopup();
  }
}
...
function renderMarkers(list) {
  ...
  Object.entries(groups).forEach(([key, events]) => {
    ...
    m.bindPopup(popupHtml(events), { closeButton: true, maxWidth: 360 });
    m._events = events;
    ...
    m.on('popupopen', () => {
      ...
      if (targetId) setActiveConcert(targetId, { scroll: false });
    });
  });
}
...
function popupHtml(events) {
  ...
  const blocks = events.map(c => {
    ...
    return `
      <div class="pop-section${many ? ' multi' : ''}" data-concert-id="${esc(c.id)}">
